...
class Pedido { 
    ...
    listarProdutos () {
            ...
            let listaItem = document.querySelector("#lista") //guardei o valor digitado no input na variavel qtd
           
    
            this.itens.map( item => {
                           
                let listagem = document.createElement("h2")  //criei o elemento no html
                
                listagem.innerHTML += (                        //atribui o valor ao elemento (li) 
                 `
                 <div id="form">
                    <div class="display" id="qtd">
                       <label>Quantidade</label> 
                       <h2>${item.Qtd}</h2>
                    </div>
                    
                    <div class="display" id="prod">
                       <label>Produto</label> 
                       <h2>${item.nome}</h2>
                    </div>

                    <div class="display" id="vlr">
                       <label>Preço</label> 
                       <h2>R$ ${item.Vlr}</h2>
                    </div>
                 </div>  
...
function enviar() {
    ...
    if (operation === cad.value && input1.value != "" && input2.value != "" ) {
        ...
        carrinho.adicionarProduto(input1.value.trim(), input2.value.trim(), input3.value.trim())
